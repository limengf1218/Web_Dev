...
export function ModerationCard({
  cardless = false,
  sections = ['title', 'tags', 'users'],
  instantRefresh = true,
}: {
  cardless?: boolean;
  sections?: Array<'title' | 'tags' | 'users'>;
  ...
  const title = (
    <Stack key="title" spacing={0} mb="md">
      <Group spacing="xs">
        <Title order={2}>{t('Content Moderation')}</Title>
        <Badge color="yellow" size="xs">
          {t('Beta')}
        </Badge>
      </Group>
      <Text color="dimmed" size="sm">
        {t(`Choose the type of content you don't want to see on the site.`)}
      </Text>
    </Stack>
  );
  ...
  const tags = (
    <Card key="tags" withBorder>
      <Card.Section withBorder inheritPadding py="xs">
        <Text weight={500}>{t('Hidden Tags')}</Text>
      </Card.Section>
      <HiddenTagsSection />
    </Card>
  );
  ...
  const users = (
    <Card key="users" withBorder>
      <Card.Section withBorder inheritPadding py="xs">
        <Text weight={500}>{t('Hidden Users')}</Text>
      </Card.Section>
      <HiddenUsersSection />
    </Card>
  );
    ...
    <Card key="nsfw" withBorder pb={0}>
      <Card.Section withBorder inheritPadding py="xs">
        <Group position="apart">
          <Text weight={500}>{t('Mature Content')}</Text>
          <SkeletonSwitch
            loading={!user}
      ...
      {!showNsfw ? (
        <Group noWrap mt="xs" pb="sm">
          ...
          <Text sx={{ lineHeight: 1.3 }}>
            {t(`By enabling Mature Content, you confirm you are over the age of 18.`)}
          </Text>
        </Group>
      ) : (
        <>
                ...
                {preferences && preferences[category.value] && !!category.children?.length && (
                  <Card.Section inheritPadding py="md">
                    <Text size="xs" weight={500} mb="xs" mt={-8} color="dimmed">
                      {t('Toggle all that you are comfortable seeing')}
                    </Text>
                    <Group spacing={5}>
                      {category.children
                        .filter((x) => !x.hidden)
  ...
  const contentSections = { title, tags, users };
