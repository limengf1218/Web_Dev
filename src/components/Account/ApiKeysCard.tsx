...
const { t } = useTranslation();
  ...
  const handleDeleteApiKey = (id: number) => {
    openConfirmModal({
      ...
      children: <Text size="sm">{t('Are you sure you want to delete this API Key?')}</Text>,
      centered: true,
      labels: { confirm: t('Delete API Key'), cancel: t("No, don't delete it") },
  ...
  return (
    <>
      <Card withBorder>
        <Stack spacing={0}>
          <Group align="start" position="apart">
            <Title order={2}>{t('API Keys')}</Title>
            <Button
              variant="outline"
          ...
          <Text color="dimmed" size="sm">
            {t('You can use API keys to interact with the site through the API as your user. These should not be shared with anyone.')}
          </Text>
        </Stack>
        <Box mt="md" sx={{ position: 'relative' }}>
          ...
          {apiKeys.length > 0 ? (
            ...
            <Paper radius="md" mt="lg" p="lg" sx={{ position: 'relative' }} withBorder>
              <Center>
                <Stack spacing={2}>
                  <Text weight="bold">{t('There are no API keys in your account')}</Text>
                  <Text size="sm" color="dimmed">
                    {t('Start by creating your first API Key to connect your apps.')}
                  </Text>
                </Stack>
              </Center>
            </Paper>
      ...
      <ApiKeyModal title={t('Create API Key')} opened={opened} onClose={toggle} />
    </>
  );
}
