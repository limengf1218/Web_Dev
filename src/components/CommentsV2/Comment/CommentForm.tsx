  ...
  if (isMuted)
    return (
      <Alert color="yellow" icon={<IconLock />}>
        <Center>{t('You cannot add comments because you have been muted')}</Center>
      </Alert>
    );
    ...
    <Form form={form} onSubmit={handleSubmit} style={{ flex: 1 }}>
      <Stack>
        <InputRTE
          ...
          hideToolbar
          placeholder={
            !data?.length ? t('Be the first to leave a comment...')as string : t('Type your comment...')as string
          }
          autoFocus={focused}
