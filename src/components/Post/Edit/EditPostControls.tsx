...
export const hiddenLabel = `Click the '${publishText}' button to make your post Public to share with the mikomiko community for comments and reactions.`;
    ...
    <Stack spacing={4}>
      <Group spacing="xs">
        {!publishedAt && (
          <Tooltip
            disabled={canPublish}
            label={t('At least one tag is required in order to publish this post to the community')}
            multiline
            width={260}
            withArrow
          >
      ...
      <Text size="xs">
        {!publishedAt ? (
          <>
            {t('Your post is currently')}{' '}
            <Tooltip label={t(hiddenLabel)} {...tooltipProps}>
          ...
          <>
            {t('Published')} <DaysFromNow date={publishedAt} />
          </>
        )}
      </Text>
    </Stack>
...
export function ManagePostMaturity() {
  ...
  return (
    ...
    //     <Text>
    //       {t('Mature')}{' '}
    //       <Tooltip label={t('Mature content may include content that is suggestive or provocative')} {...tooltipProps}>
