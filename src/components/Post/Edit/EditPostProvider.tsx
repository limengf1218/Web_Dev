                ...
                .map(async (data) => {
                  await handleUpload({ postId, modelVersionId, ...data }, async (created) => {
                    ...
                    set((state) => {
                      ...
                      state.images[index] = {
                        type: 'image',
                        data: { ...created, previewUrl: data.url } as PostEditImage,
                      };
                    });
                    try {
                      const result = await ingestImage(created as PostEditImage);
...
export const EditPostProvider = ({
  ...
  const handleUpload = async (
    ...
    await upload(file, async (result) => {
      ...
      mutateAsync({ ...data, url: id, postId, modelVersionId }).then((response) => cb(response as PostImage));
    });
  };
