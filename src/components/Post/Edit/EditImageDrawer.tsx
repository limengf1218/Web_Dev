...
const { t } = useTranslation();
  ...
  const handleSubmit = (data: z.infer<typeof schema>) => {
    ...
    mutate(payload, {
      onSuccess: (response) => {
        showSuccessNotification({ message: t('Image details saved successfully') });
          ...
          <Stack spacing="xl" pt="md" pb={4}>
            ...
            <Input.Wrapper label="Tags">
              <Group spacing={4}>
                {hasTags ? (
                  ...
                  <Alert color="yellow">
                    {t('There are no tags associated with this image yet. Tags will be assigned to this image soon.')}
                  </Alert>
                )}
              </Group>
            </Input.Wrapper>
            <Input.Wrapper label="Resources">
              {!!image.resourceHelper.length ? (
                  ...
                  <DismissibleAlert
                    ...
                    content={
                      <>
                        ...
                        {t('to automatically detect all the resources used in your images.')}
                      </>
                    }
                  />
                              ...
                              {resource.modelVersionId ? (
                                ...
                                <Group spacing={4}>
                                  <Popover width={300} withinPortal withArrow>
                                    ...
                                    <Popover.Dropdown>
                                      <Text>
                                        {t('The detected image resource was not found in our system')}
                                      </Text>
                                    </Popover.Dropdown>
                                  </Popover>
                                  <Text size="sm" weight={500} lineClamp={1}>
                ...
                <Alert color="yellow">
                  {t('We could not detect any resources associated with this image. If this image is based on a model hosted on Civitai, try creating this post from the model detail page. For automatic image resource detection, try installing')}{' '}
                  <Text
                    component="a"
                    href="https://github.com/civitai/sd_civitai_extension"
                    target="_blank"
            ...
            <Grid gutter="xs">
              ...
              <Grid.Col span={6}>
                <InputNumber name="meta.cfgScale" label={t('Guidance scale')} min={0} max={30} />
              </Grid.Col>
              <Grid.Col span={6}>
