...
const { t } = useTranslation();
  ...
  if (!images?.length)
    return (
      <Alert>{t('Due to your filter settings, we could not display any images from this post')}</Alert>
    );
  ...
  return (
    <Stack>
      ...
      />
      {remainingImages > 0 && (
        <Button onClick={() => setShowMore(true)}>{t('Load')} {remainingImages} more images</Button>
      )}
    </Stack>
  );
}
