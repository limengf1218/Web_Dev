...
export function ModelUpsertForm({ model, children, onSubmit }: Props) {
  ...
  const upsertModelMutation = trpc.model.upsert.useMutation({
    ...
    onError: (error) => {
      showErrorNotification({ error: new Error(error.message), title: t('Failed to save model') as string });
    },
  });
  ...
  return (
    <Form form={form} onSubmit={handleSubmit}>
      <Grid gutter="xl">
        <Grid.Col span={12}>
          <Stack>
            <InputText name="name" label={t("Name")} placeholder="Name" withAsterisk />
            <Stack spacing={5}>
              <Group spacing="sm" grow>
                ...
                />
                {type === 'Checkpoint' && (
                  <Input.Wrapper label={t('Checkpoint Type')} withAsterisk>
                    <InputSegmentedControl
                      name="checkpointType"
            ...
            <InputTags
              name="tagsOnModels"
              label="Tags"
              description={t('Search or create tags for your model')}
            ...
            <InputRTE
              name="description"
              label={t('About your model')}
              description={t('Tell us what your model does')}
              includeControls={[
                'heading',
                'formatting',
                'list',
          ...
          <Stack>
                ...
                <Grid.Col xs={12} sm={6}>
                  <Stack spacing="xs">
                    <Text size="md" weight={500} sx={{ lineHeight: 1.2 }} mb="xs">
                      {t(`When using this model, I give permission for users to:`)}
                    </Text>
                    <InputCheckbox name="allowNoCredit" label={t('Use without crediting me')} />
                    <InputCheckbox name="allowDerivatives" label={t('Share merges of this model')} />
                    {allowDerivatives && (
                      <InputCheckbox
                        name="allowDifferentLicense"
                        label={t('Use different permissions on merges')}
                      />
                    )}
                  </Stack>
                </Grid.Col>
                      ...
                      data={[
                        {
                          ...
                          label: (
                            <Group>
                              <IconCurrencyDollarOff size={16} /> {t('None1')}
                            </Group>
                          ),
                        },
                        {
                          ...
                          label: (
                            <Group>
                              <IconPhoto size={16} /> {t('Sell generated images')}
                            </Group>
                          ),
                        },
                        {
                          ...
                          label: (
                            <Group>
                              <IconBrush size={16} /> {t('Use on generation services')}
                            </Group>
                          ),
                        },
                        {
                          ...
                          label: (
                            <Group>
                              <IconShoppingCart size={16} /> {t('Sell this model or merges')}
                            </Group>
                          ),
                        },
                      ]}
            ...
            <Paper radius="md" p="xl" withBorder>
              <Stack>
                ...
                <InputCheckbox
                  name="poi"
                  label={t('Depicts an actual person')}
                  description={t('For Example: Tom Cruise or Tom Cruise as Maverick')}
                />
                {/* <InputCheckbox name="nsfw" label={t('Is intended to produce mature themes only')} /> */}
              </Stack>
            </Paper>
                ...
                <Alert color="red" pl={10}>
                  <Group noWrap spacing={10}>
                    ...
                    <Text size="xs" sx={{ lineHeight: 1.2 }}>
                      {t('Mature content depicting actual people is not permitted.')}
                    </Text>
                  </Group>
                </Alert>
                <Text size="xs" color="dimmed" sx={{ lineHeight: 1.2 }}>
                  {t('Please revise the content of this listing to ensure no actual person is depicted in an mature context out of respect for the individual.')}
                </Text>
              </>
            )}
          </Stack>
