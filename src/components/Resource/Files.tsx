    ...
    <Stack>
      ...
      >
        <Group position="center" spacing="xl" style={{ minHeight: 120, pointerEvents: 'none' }}>
          ...
          <Stack spacing={8}>
            <Text size="xl" inline>
              {t('Drop your files here or click to select')}
            </Text>
            <Text size="sm" color="dimmed" inline>
              {`附加至 ${maxFiles} 文件. 接受的文件类型: ${fileExtensions.join(', ')}`}
            </Text>
          </Stack>
        </Group>
      </Dropzone>
  ...
  const deleteFileMutation = trpc.modelFile.delete.useMutation({
    ...
    onError() {
      showErrorNotification({
        error: new Error(t('Could not delete file, please try again')as string),
      });
    },
  });
      ...
      <Stack spacing={4} pb="xs">
        <Group position="apart" spacing={4} noWrap>
          ...
          {!versionFile.isUploading && (
            <Tooltip label={t('Remove file')} position="left">
              <ActionIcon
                color="red"
          ...
          <Stack spacing={4} sx={{ flex: '1 !important' }}>
            <Group spacing={4}>
              <Progress
                ...
                sx={{ flex: 1 }}
              />
              <Tooltip label={t('Cancel upload')} position="left">
                <ActionIcon
                  color="red"
                  onClick={() => {
                    abort(uuid);
    ...
    case 'aborted':
      return (
        <Group position="apart" noWrap spacing="xs" sx={{ width: '100%' }}>
          <Group spacing="xs">
            <IconBan color="red" />
            <Text size="sm">{t('Aborted upload')}</Text>
          </Group>
          <Tooltip label={t('Remove file')} position="left">
    ...
    case 'error':
      return (
        <Group position="apart" noWrap spacing="xs" sx={{ width: '100%' }}>
          <Group spacing="xs">
            <IconBan color="red" />
            <Text size="sm">{t('Failed to upload')}</Text>
          </Group>
          <Tooltip label={t('Retry upload')} position="left">
    ...
    case 'success':
      return (
        <>
          ...
          <Text size="sm">{t('Upload completed')}</Text>
        </>
      );
    case 'pending':
      return (
        <Group position="apart" noWrap spacing="xs" sx={{ width: '100%' }}>
          <Group spacing="xs">
            <IconCloudUpload />
            <Text size="sm">{t('Pending upload')}</Text>
          </Group>
          <Tooltip label={t('Remove file')} position="left">
            <ActionIcon color="red" onClick={handleRemoveFile}>
              <IconTrash />
...
const { t } = useTranslation();
  ...
  return (
    <Stack>
      <Select
        label={t('File Type')}
        placeholder={t('Select a type') as string}
      ...
      {versionFile.type === 'Model' && versionFile.modelType === 'Checkpoint' && (
        <>
          <Select
            label={t("Model Size")}
            placeholder={t('Pruned or Full') as string}
            ...
            return openConfirmModal({
              title: (
                <Group spacing="xs">
                  ...
                  <Text size="lg">{t('Missing files')}</Text>
                </Group>
              ),
              children:
                t('You have not uploaded any files. You can continue without files, but you will not be able to publish your model. Are you sure you want to continue?'),
              labels: { cancel: 'Cancel', confirm: t('Continue') },
