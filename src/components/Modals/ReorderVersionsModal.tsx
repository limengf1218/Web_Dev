...
const { t } = useTranslation();
  ...
  const reorderMutation = trpc.model.reorderVersions.useMutation({
    ...
    onError(error, payload, context) {
      ...
      showErrorNotification({ error: new Error(error.message), title: t('Failed to save')as string });
    },
  });
  const handleSave = () => {
  ...
  return (
    <Modal
      ...
      title={
        <Stack spacing={0}>
          <Title order={3}>{t('Rearrange versions')}</Title>
          <Text size="sm" color="dimmed">
            {t('Drag and drop the versions to set their order')}
          </Text>
        </Stack>
      }
