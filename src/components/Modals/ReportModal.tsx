    ...
    const { mutate, isLoading: isLoading } = trpc.report.create.useMutation({
      onMutate() {
        showNotification({
          ...
          disallowClose: true,
          autoClose: false,
          message: t('Sending report...'),
        });
      },
      async onSuccess(_, variables) {
        showSuccessNotification({
          title: t('Resource reported')as string,
          message: t('Your request has been received')as string,
        });
        context.close();
      ...
      onError(error) {
        showErrorNotification({
          ...
          title: t('Unable to send report')as string,
          reason: error.message ?? t('An unexpected error occurred, please try again'),
        });
      },
      onSettled() {
            ...
            >
              {reports
                ...
                .map(({ reason, label }, index) => (
                  <Radio key={index} value={reason} label={t(label)} />
                ))}
            </Radio.Group>
          )
        )}
