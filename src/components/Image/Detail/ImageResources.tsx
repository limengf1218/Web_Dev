...
const { t } = useTranslation();
  ...
  const handleRemoveResource = (resourceId: number) => {
    ...
    openConfirmModal({
      ...
      children:
        t('Are you sure you want to remove this resource from this image? This action is destructive and cannot be reverted.'),
      labels: { confirm: t('Yes, remove it'), cancel: 'Cancel' },
          ...
          { id: resourceId },
          {
            async onSuccess() {
              showSuccessNotification({
                title: t('Successfully removed resource')as string,
                message: t('The resource was removed from the image')as string,
              });
              await queryUtils.image.getResources.invalidate({ id: imageId });
            },
            onError(error) {
              showErrorNotification({
                title: t('Unable to remove resource')as string,
  ...
  return (
    <Stack spacing={4}>
      {isLoading ? (
        ...
        <Alert>{t('There are no resources associated with this image')}</Alert>
      ) : (
