...
export const PermissionIndicator = ({ permissions, size = 20, spacing = 2, ...props }: Props) => {
  ...
  const icons = [
    !allowNoCredit && { label: t('Creator credit required'), icon: <IconUserCheck {...iconProps} /> },
    !canSellImages && { label: t('No selling images'), icon: <IconPhotoOff {...iconProps} /> },
    !canRent && { label: t('No generation services'), icon: <IconBrushOff {...iconProps} /> },
    !canSell && { label: t('No selling models'), icon: <IconShoppingCartOff {...iconProps} /> },
    !allowDerivatives && { label: t('No sharing merges'), icon: <IconExchangeOff {...iconProps} /> },
    !allowDifferentLicense && {
      label: t('Same permissions required'),
  ...
  return (
    <Popover withArrow>
      ...
      <Popover.Dropdown>
        <Text weight={500}>{t('This model permits users to:')}</Text>
        <List
          size="xs"
          styles={{
