        ...
        for (const id in dict) {
          ...
          if (hasCompleted) {
            ...
            if (activity.type === 'resources:add') {
              showNotification({
                title: 'mikomiko Link',
                message: `${inError ? 'Failed ' : ''}Added ${activity.resource.modelName
                  } to SD Instance`,
...
const Provider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getWorker = () => {
    ...
    const handleMessage = (msg: string) => {
      showNotification({ id: msg, message: msg, title: 'mikoiko Link', color: 'blue' });
    };
  ...
  const runCommand = async (command: CommandRequest, timeout = 0) => {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        reject(new Error(t('Request timed out') as string));
      }, timeout);
    });
  ...
  return flags.civitaiLink ? (
    ...
    <CivitaiLinkCtx.Provider
      value={{
        ...
        resources: [],
        error: t('mikomiko Link is not enabled') as string,
