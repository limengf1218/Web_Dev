...
const { t } = useTranslation();
  ...
  const { mutate, isLoading } = trpc.question.upsert.useMutation({
    ...
    onError(error: TRPCClientErrorBase<DefaultErrorShape>) {
      ...
      showNotification({
        title: t('Could not save question'),
    ...
    return (
      <Container size="xl" p="xl">
        <Stack align="center">
          ...
          <Title order={1}>{t('Restricted Area')}</Title>
          <Text size="xl">
            {t('You are not able to create/edit a question because your account has been muted')}
          </Text>
          <Link href="/" passHref>
            <Button component="a">{t('Go back home')}</Button>
          </Link>
        </Stack>
      </Container>
    );
  ...
  return (
    <Container>
      <Group spacing="lg" mb="lg">
        ...
        <Title order={3}>{question ? t('Editing question') : t('Ask a question')}</Title>
      </Group>
        ...
        <Grid gutter="xl">
          <Grid.Col lg={8}>
            <Paper radius="md" p="xl" withBorder>
              <Stack>
                <InputText name="title" label={t("Title")} withAsterisk />
                <InputRTE
                  name="content"
                  label={t("Content")}
                  withAsterisk
            ...
            <Paper radius="md" p="xl" withBorder>
              <Stack>
                <InputMultiSelect
                  ...
                  description={t('Please add your tags')}
                  creatable
