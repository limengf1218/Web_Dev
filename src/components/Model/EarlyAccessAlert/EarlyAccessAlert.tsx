    ...
    onSuccess() {
      showSuccessNotification({
        message: !alreadyNotifying
          ? t('You have been removed from the notification list')
          : t('You will be notified when this is available for download'),
      });
    },
  ...
  return (
    <AlertWithIcon color="green" iconColor="green" icon={<IconAlertCircle />}>
      ...
      <Text variant="link" component={NextLink} href="/pricing">
        {t('Supporter Tier members')}
      </Text>
      {t(`. It will be publicly available in`)}
      ...
      <LoginRedirect reason="notify-version">
        ...
        >
          {alreadyNotifying
            ? t('Remove me from this notification.')
            : t(`Notify me when it's available.`)}
        </Text>
      </LoginRedirect>
    </AlertWithIcon>
  );
