...
const { t } = useTranslation();
  ...
  const handlePublishClick = async () => {
    try {
      ...
      showErrorNotification({
        ...
        title: t('Error publishing model')as string,
        reason: t('Something went wrong while publishing your model. Please try again later.')as string,
      });
    }
  ...
  const modelDetails: DescriptionTableProps['items'] = [
    ...
    { label: t('Uploaded'), value: formatDate(version.createdAt) },
    {
      label: t('Base Model'),
      value: <Text>{version.baseModel}</Text>,
    },
    { label: 'Steps', value: version.steps?.toLocaleString() ?? 0, visible: !!version.steps },
    { label: t('Epoch'), value: version.epochs?.toLocaleString() ?? 0, visible: !!version.epochs },
    {
        ...
        <Stack>
          ...
          {showRequestReview ? (
            ...
            <Stack spacing={4}>
              <Group spacing="xs" style={{ alignItems: 'flex-start', flexWrap: 'nowrap' }}>
                    ...
                    <Stack sx={{ flex: 1 }} spacing={4}>
                      ...
                      >
                        <Text align="center">
                         {primaryFile ? (
                         <>
                         {t('Download')} (<span>{formatKBytes(primaryFile.sizeKB)}</span>)
                         </>
                         ) : (
                         t('No file')
                          )}
                  ...
                  <Stack sx={{ flex: 1 }} spacing={4}>
                    <JoinPopover>
                      <Button leftIcon={<IconDownload size={16} />} disabled={archived}>
                        <Text align="center">
                          {`下载 (${formatKBytes(primaryFile?.sizeKB ?? 0)})`}
                        </Text>
                      </Button>
                    </JoinPopover>
                    {/* {primaryFileDetails} */}
                ...
                <Tooltip label={t('Share')} position="top" withArrow>
                  <div>
                ...
                <Tooltip label={isFavorite ? t('Unlike') : t('Like')} position="top" withArrow>
                  <div>
          ...
          >
                  ...
                  {hasFiles ? (
                    ...
                    <Center p="xl">
                      <Text size="md" color="dimmed">
                        {t('This version is missing files')}
                      </Text>
                    </Center>
                  )}
                </Stack>
            ...
            {version.description && (
              <Accordion.Item value="version-description">
                <Accordion.Control>{t(`About this version`)}</Accordion.Control>
                <Accordion.Panel px="sm" pb="sm">
                  <Stack spacing={4}>
                    {version.description && (
          ...
          <Group position="apart" align="flex-start" style={{ flexWrap: 'nowrap' }}>
            {model.type === 'Checkpoint' && (
              <Group spacing={4} noWrap style={{ flex: 1, overflow: 'hidden' }} align="flex-start">
                ...
                >
                  {t('License')}{model.licenses.length > 0 ? 's' : ''}:
                </Text>
                <Stack spacing={0}>
                  <Text
                    component="a"
          ...
          {hasPendingClaimReport && (
            <AlertWithIcon icon={<IconMessageCircle2 />}>
              {t(`A verified artist believes this model was fine-tuned on their art. We're discussing this with the model creator and artist`)}
            </AlertWithIcon>
          )}
        </Stack>
      </Grid.Col>
