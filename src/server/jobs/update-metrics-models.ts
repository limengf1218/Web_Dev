  ...
  async () => {
    ...
    const updateModelMetrics = async (since: Date) => {
      ...
      async function updateVersionDownloadMetrics() {
        const affectedModelVersionsResponse = await clickhouse?.query({
          query: `
            ...
            (
                SELECT DISTINCT modelVersionId,time
            ...
            SELECT
                ...
                SUM(if(mv.time >= subtractDays(now(), 1), 1, null)) AS downloads24Hours,
                SUM(if(mv.time >= subtractDays(now(), 7), 1, null)) AS downloads1Week,
                SUM(if(mv.time >= subtractMonths(now(), 1), 1, null)) AS downloads1Month,
                SUM(if(mv.time >= subtractYears(now(), 1), 1, null)) AS downloads1Year,
                COUNT() AS downloadsAll
            FROM CTE_AffectedModelVersions mv
            GROUP BY mv.modelVersionId
          `,
          format: 'JSONEachRow',
        });
        ...
        for (let batchNumber = 0; batchNumber < batchCount; batchNumber++) {
          ...
          await dbWrite.$executeRaw`
            ...
            ON CONFLICT ("modelVersionId", timeframe) DO UPDATE
              SET "downloadCount" = "ModelVersionMetric"."downloadCount" + EXCLUDED."downloadCount";
          `;
        }
      }
      ...
      async function updateVersionRatingMetrics() {
        ...
        const affectedModelVersionsId = (affectedModelVersions.map((x) => x.modelVersionId));
        ...
        await dbWrite.$executeRaw`
          ...
          FROM (
              ...
              AND r."modelVersionId"::int = ANY (${affectedModelVersionsId}::int[])
          ...
          ON CONFLICT ("modelVersionId", timeframe) DO UPDATE 
            SET "ratingCount" = "ModelVersionMetric"."ratingCount" + EXCLUDED."ratingCount", 
                rating = "ModelVersionMetric".rating + EXCLUDED.rating;
        `;
      }
      ...
      async function updateVersionFavoriteMetrics() {
        ...
        const affectedModelsId= (effectedModels.map((x) => x.modelId));
        ...
        await dbWrite.$executeRaw`
          ...
          FROM (
              ...
              AND f."modelId"::int = ANY (${affectedModelsId}::int[])
          ) f
          ...
          ON CONFLICT ("modelVersionId", timeframe) DO UPDATE 
            SET "favoriteCount" = "ModelVersionMetric"."favoriteCount" + EXCLUDED."favoriteCount";
        `;
      }
      ...
      async function updateVersionCommentMetrics() {
        ...
        const affectedModelsId = (affectedModels.map((x) => x.modelId));
        ...
        await dbWrite.$executeRaw`
          ...
          FROM (
              ...
              AND c."modelId"::int = ANY (${affectedModelsId}::int[])
          ) c
          ...
          GROUP BY mv.id, tf.timeframe
          ON CONFLICT ("modelVersionId", timeframe) DO UPDATE 
            SET "commentCount" = "ModelVersionMetric"."commentCount" + EXCLUDED."commentCount";
        `;
      }
