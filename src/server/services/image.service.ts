...
export const ingestImage = async ({
  ...
  if (!callbackUrl) {
    ...
    let config = new $OpenApi.Config({
      ...
      accessKeySecret: env.S3_UPLOAD_SECRET,
    });

    config.endpoint = env.IMAGE_SCANNING_ENDPOINT;
    let image_scanner_client = new imm20200930(config);

    let detectImageLabelsRequest = new $imm20200930.DetectImageLabelsRequest({
      projectName: "test-project",
      sourceURI: "oss://" + env.S3_IMAGE_UPLOAD_BUCKET + "/" + url,
    });

    let runtime = new $Util.RuntimeOptions({ });

    try {
      const image_scanner_res = await image_scanner_client.detectImageLabelsWithOptions(detectImageLabelsRequest, runtime);
      const respondBody = image_scanner_res.body.labels

      if (respondBody) {
        for (const label_info of respondBody) {
          const label_name = label_info.labelName

          const tag_id = await dbWrite.tag.upsert({
            where: { name: label_name },
            update:{},
            create:{ name: label_name!, target: "Image" },
            select: { id: true }
          });

          await dbWrite.tagsOnImage.upsert({
            where: {
              tagId_imageId: {
                tagId: tag_id.id,
                imageId: id,
