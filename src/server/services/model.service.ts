...
export const upsertSeedModel = async ({
  id,
  tagsOnModels,
  userId,
  stats,
...
export const removeTags = async ({ id: modelId, tags }: DeleteModelTagsInput) => {
  if (typeof tags === 'undefined') return;
  for (const t of tags) {
    await dbWrite.$transaction(async (tx) => {
      const result = await tx.tag.findUnique({
        where: {
          name: t,
        },
        select: { id: true },
      });
      if (result === null) throw new Error(`tagId not found for ${t}`);
      await tx.tagsOnModels.delete({
        where: {
          modelId_tagId: {
            modelId: modelId,
            tagId: result.id,
