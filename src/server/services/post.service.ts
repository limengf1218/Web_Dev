...
export const createSeedPost = async ({
  userId,
  tag,
  ...data
}: PostCreateInput & { id?: number; userId: number }) => {
  let rawResult;
  if (!data.id) {
    rawResult = await dbWrite.post.create({
      data: { ...data, userId, tags: tag ? { create: { tagId: tag } } : undefined },
      select: editPostSelect,
    });
  } else {
    rawResult = await dbWrite.post.upsert({
      where: { id: data.id },
      update: { ...data, userId, tags: tag ? { create: { tagId: tag } } : undefined },
      create: { ...data, userId, tags: tag ? { create: { tagId: tag } } : undefined },
      select: editPostSelect,
    });
  }
  const imageIds = rawResult.images.map((x) => x.id);
...
export const publishPost = ({ id, ...data }: PostUpdateInput) => {
  return dbWrite.post.updateMany({
    where: { id, publishedAt: undefined },
...
export const addSeedPostImage = async ({
  modelVersionId,
  meta,
  stats,
  ...image
}: AddPostImageInput & { id?: number; userId: number; stats?: any }) => {
  const partialResult = await dbWrite.image.upsert({
    where: { id: image.id },
    update: {
      ...image,
      meta: (meta as Prisma.JsonObject) ?? Prisma.JsonNull,
  ...
  if (stats) {
    await Promise.all(
      Object.values(MetricTimeframe).map(async (timeframe) => {
        await dbWrite.imageMetric.upsert({
          where: {
            imageId_timeframe: {
              imageId: image.id!,
              timeframe: timeframe,
    ...
    await Promise.all(
      Object.values(MetricTimeframe).map(async (timeframe) => {
        await dbWrite.postMetric.upsert({
          where: {
            postId_timeframe: {
              postId: image.postId!,
              timeframe: timeframe,
          ...
          update: {
            cryCount: { increment: stats.cryCount },
            laughCount: { increment: stats.laughCount },
            likeCount: { increment: stats.likeCount },
            dislikeCount: { increment: stats.dislikeCount },
            heartCount: { increment: stats.heartCount },
            commentCount: { increment: stats.commentCount },
          },
          create: {
            postId: image.postId!,
            timeframe: timeframe,
