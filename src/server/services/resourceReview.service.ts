...
export const upsertSeedResourceReview = (input: UpsertResourceReviewInput & { userId: number }) => {
  return dbWrite.resourceReview.upsert({
    where: {
      modelVersionId_userId: { modelVersionId: input.modelVersionId, userId: input.userId },
    },
    update: input,
    create: input,
    select: { id: true },
