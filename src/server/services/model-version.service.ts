import { MetricTimeframe, ModelStatus, ModelVersionEngagementType, Prisma } from '@prisma/client';
...
export const upsertSeedModelVersion = async ({ id, modelId, stats, ...data }: ModelVersionUpsertInput & { stats?: any }) => {
  const existingVersions = await dbRead.modelVersion.findMany({ where: { modelId } });
  ...
  if (stats) {
    await Promise.all(
      Object.values(MetricTimeframe).map(async (timeframe) => {
        await dbWrite.modelVersionMetric.upsert({
          where: {
            modelVersionId_timeframe: {
              modelVersionId: id!,
              timeframe: timeframe,
            }
          },
