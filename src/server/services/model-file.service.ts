...
export const createSeedFile = <TSelect extends Prisma.ModelFileSelect>({
  select,
  ...data
}: ModelFileCreateInput & { select: TSelect } & { id: number }) => {
  const file = prepareFile(data);

  return dbWrite.modelFile.upsert({
    where: { id: data.id },
    update: { ...file },
    create: { ...file, modelVersionId: data.modelVersionId },
    select,
  });
};
...
export const upsertSeedModelFileHash = (input: ModelFileHashUpsertInput) => {
  return dbWrite.modelFileHash.upsert({
    where: {
      fileId_type: { fileId: input.fileId, type: input.type },
    },
    update: input,
    create: input,
  });
