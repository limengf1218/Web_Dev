...
export const createSeedPostHandler = async ({
  input,
  ctx,
}: {
  input: PostCreateInput & { id?: number; userId: number };
  ctx: DeepNonNullable<Context>;
}) => {
  try {
    const post = await createSeedPost({ ...input });
    // await ctx.track.post({
    //   type: 'Create',
    //   nsfw: post.nsfw,
    //   postId: post.id,
...
export const publishPostHandler = async ({ input }: { input: PostUpdateInput }) => {
  try {
...
export const addSeedPostImageHandler = async ({
  input,
}: {
  input: AddPostImageInput & { id?: number; userId: number; stats?: any };
}) => {
  try {
    return await addSeedPostImage({ ...input });
  } catch (error) {
    console.error(error);
    if (error instanceof TRPCError) throw error;
    else throw throwDbError(error);
