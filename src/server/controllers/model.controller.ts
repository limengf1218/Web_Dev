...
export const upsertSeedModelHandler = async ({
  input,
  ctx,
}: {
  input: ModelUpsertInput & { userId: number; stats?: any };
  ctx: DeepNonNullable<Context>;
}) => {
  try {
    const model = await upsertSeedModel({ ...input });
    if (!model) throw throwNotFoundError(`No model with id ${input.id}`);
...
export const removeTagsHandler = async ({
  input,
  ctx,
}: {
  input: DeleteModelTagsInput;
  ctx: DeepNonNullable<Context>;
}) => {
  try {
    const { id: modelId, tags } = input;
    return await removeTags(input);
  } catch (error) {
    console.error(error);
  }
};
