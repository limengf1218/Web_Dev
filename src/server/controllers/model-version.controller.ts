...
export const upsertSeedModelVersionHandler = async ({
  input,
  ctx,
}: {
  input: ModelVersionUpsertInput & { stats?: any };
  ctx: DeepNonNullable<Context>;
}) => {
  try {
    const version = await upsertSeedModelVersion({ ...input });
    if (!version) throw throwNotFoundError(`No model version with id ${input.id}`);
