...
export const upsertSeedUserHandler = async ({
  input,
}: {
  input: GetByUsernameSchema & { image?: string };
}) => {
  try {
    const user = await upsertSeedUser({ ...input });
    return user;
  } catch (error) {
    if (error instanceof TRPCError) throw error;
    else throw throwDbError(error);
...
export const updateUserHandler = async ({
  ...
  if (!id) throw throwBadRequestError('Invalid ID');
  if (id !== currentUser.id && !currentUser.isModerator) throw throwAuthorizationError();
    ...
    const cosmetics = user.cosmetics.reduce(
      (acc, { obtainedAt, cosmetic }) => {
        ...
        if (type === CosmeticType.Badge)
          acc.badges.push({
            ...rest,
            data: data as BadgeCosmetic['data'],
            obtainedAt,
          });
        else if (type === CosmeticType.NamePlate)
          acc.nameplates.push({
            ...rest,
            data: data as NamePlateCosmetic['data'],
            obtainedAt,
          });
...
export const toggleArticleEngagementHandler = async ({
  ...
  try {
    const on = await toggleUserArticleEngagement({
      ...input,
      userId: ctx.user.id,
    });
...
export const getUserbyUsernameHandler = async ({ input }: { input: GetByUsernameSchema }) => {
  try {
    const { username } = input;
