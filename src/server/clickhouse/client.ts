...
export class Tracker {
  ...
  private async track(table: string, custom: object) {
    ...
    // const response = await fetch(`${env.CLICKHOUSE_TRACKER_URL}/track/${table}`, {
    //   method: 'POST',
    ...
    const response = await fetch(
        `${env.CLICKHOUSE_TRACKER_URL}/api/internal/track-clickhouse?token=${env.WEBHOOK_TOKEN}&table=${table}`,
        {
          method: 'POST',
          body: JSON.stringify(data),
        }
