...
const { t } = useTranslation();
  ...
  const Versions = loadingVersions ? (
    ...
    <Select
      placeholder={t('Showing all versions')as string}
  ...
  const UserReview = !currentUser ? (
    <Alert>{t('You must be logged in to leave a review')}</Alert>
  ) : !modelVersionId ? (
    <Alert>{t('Select a model version to leave a review')}</Alert>
    ...
    <Stack spacing={4}>
      <Text weight={500}>{t('Leave a review')}</Text>
  ...
  return (
    <>
      <Meta
        title={`${model?.name} Reviews | Rated ${ratingAverage} Stars by ${ratingCount} Users on mikomiko`}
        description={`Explore user reviews of the ${model?.name} AI model on mikomiko, rated ${ratingAverage} stars by ${ratingCount} users, and see how it has helped others bring their creative visions to life`}
      />
      <Container size="md">
        {Model}
        <Divider my="md" />
            ...
            <Center>
              <Group spacing="xs">
                ...
                <Text size="lg" color="dimmed">
                  {t('Reviews are turned off for this model.')}
                </Text>
              </Group>
            </Center>
          </Paper>
    ...
    <Card withBorder shadow="sm">
      <Stack key={review.id} spacing={4}>
        ...
        <Group spacing="xs">
          ...
          {(review.exclude || isCreator) && <Badge color="red">{t('Excluded from average')}</Badge>}
        </Group>
        {review.details && (
