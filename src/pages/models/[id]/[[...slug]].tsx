...
const { t } = useTranslation();
  ...
  const deleteMutation = trpc.model.delete.useMutation({
    async onSuccess(_, { permanently }) {
      ...
      showSuccessNotification({
        title: t('Successfully deleted the model')as string,
        message: t('Your model has been deleted')as string,
      });
      closeAllModals();
    },
    onError(error) {
      showErrorNotification({
        ...
        title: t('Could not delete model')as string,
  ...
  const handleDeleteModel = (options?: { permanently: boolean }) => {
    ...
    openConfirmModal({
      ...
      children: permanently
        ? t('Are you sure you want to permanently delete this model? This action is destructive and cannot be reverted.')
  ...
  const deleteVersionMutation = trpc.modelVersion.delete.useMutation({
    ...
    onError(error, _variables, context) {
      showErrorNotification({
        error: new Error(error.message),
        title: t('Unable to delete version')as string,
        reason: t('An unexpected error occurred, please try again')as string,
      });
  ...
  const handleDeleteVersion = (versionId: number) => {
    openConfirmModal({
      ...
      children:
        t('Are you sure you want to delete this version? This action is destructive and cannot be reverted.'),
      centered: true,
      labels: { confirm: t('Delete Version'), cancel: t("No, don't delete it") },
  ...
  const handleChangeMode = async (mode: ModelModifier | null) => {
    ...
    changeModeMutation.mutate(
      { id, mode },
      {
        ...
        onError(error) {
          showErrorNotification({
            title: t('Unable to change mode')as string,
            error: new Error(error.message),
          });
  ...
  const meta = (
    <Meta
      ...
      } | Stable Diffusion ${getDisplayName(model.type)} | mikomiko`}
      ...
      <Container size="xl">
        <Stack spacing="xl">
          ...
          <Stack spacing="xs">
            <Stack spacing={4}>
                  ...
                  <Menu.Dropdown>
                    ...
                    {currentUser && (
                      <>
                        ...
                        >
                          {t('Hide content with these tags')}
                        </Menu.Item>
                      </>
                    )}
                    {isOwner && (
                      <ToggleLockModel modelId={model.id} locked={model.locked}>
                        {({ onClick }) => (
                          <Menu.Item
                            ...
                            onClick={onClick}
                          >
                            {model.locked ? t('Unlock') : t('Lock')} {t('model discussion')}
                          </Menu.Item>
                        )}
                      </ToggleLockModel>
                    )}
              ...
              <Group spacing={4}>
                <Text size="xs" color="dimmed">
                  {t('Updated:')} {formatDate(model.updatedAt)}
                </Text>
                {category && (
                  <>
            ...
            {(model.status === ModelStatus.Unpublished || deleted) && (
              <Alert color="red">
                <Group spacing="xs" noWrap align="flex-start">
                  ...
                  <Text size="md">
                    {t('This model has been')} {deleted ? 'deleted' : 'unpublished'} {t('and is not visible to the community.')}
                  </Text>
                </Group>
              </Alert>
            )}
            {model.status === ModelStatus.UnpublishedViolation && !model.meta?.needsReview && (
              <Alert color="red">
                <Group spacing="xs" noWrap align="flex-start">
                  ...
                  <Text size="sm" mt={-3}>
                    {t('This model has been unpublished due to a violation of our')}{' '}
                    <Text component="a" variant="link" href="/content/tos" target="_blank">
                      {t('guidelines')}
                    </Text>{' '}
                    {t('and is not visible to the community.')}{' '}
                    {unpublishedReason && unpublishedMessage ? unpublishedMessage : null} {t('If you adjust your model to comply with our guidelines, you can request a review from one of our moderators. If you believe this was done in error, you can')}{' '}
            ...
            {model.status === ModelStatus.UnpublishedViolation && model.meta?.needsReview && (
              <Alert color="yellow">
                <Group spacing="xs" noWrap>
                  ...
                  <Text size="md">
                    {t('This model is currently being reviewed by our moderators. It will be visible to the community once it has been approved.')}
                  </Text>
                </Group>
              </Alert>
            )}
            {inaccurate && (
              <Alert color="yellow">
                <Group spacing="xs" noWrap align="flex-start">
                  ...
                  <Text size="md">
                    {t('The images on this')} {splitUppercase(model.type).toLowerCase()} {t('are inaccurate. Please submit reviews with images so that we can improve this page.')}
                  </Text>
                </Group>
              </Alert>
            )}
            {(model.mode === ModelModifier.TakenDown || model.mode === ModelModifier.Archived) && (
              <AlertWithIcon color="blue" icon={<IconExclamationMark />} size="md">
                {model.mode === ModelModifier.Archived
                  ? t('This model has been archived and is not available for download. You can still share your creations with the community.')
                  : t('The visual assets associated with this model have been taken down. You can still download the resource, but you will not be able to share your creations.')}
              </AlertWithIcon>
            )}
          </Stack>
          <Group spacing={4} noWrap>
            {isOwner ? (
              <>
                <ButtonTooltip label={t('Add Version')}>
                  <ActionIcon
                    component={NextLink}
                ...
                {versionCount > 1 && (
                  <ButtonTooltip label={t('Rearrange Versions')}>
          ...
          <Container size="xl" my="xl">
            <Stack spacing="md">
              <Group ref={discussionSectionRef} sx={{ justifyContent: 'space-between' }}>
                <Group spacing="xs">
                  <Title order={2}>{t('Discussion')}</Title>
                  {canDiscuss ? (
                    <>
                    ...
                    !isMuted && (
                      <JoinPopover message={t('You must be a Supporter Tier member to join this discussion')}>
                        <Button
            ...
            <Center>
              <Group spacing="xs">
                ...
                <Text size="lg" color="dimmed">
                  {t('Discussion is turned off for this model.')}
                </Text>
              </Group>
            </Center>
          </Paper>
