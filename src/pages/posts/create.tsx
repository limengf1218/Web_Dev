  ...
  return (
    <Container size="xs">
      <Group spacing="xs">
        ...
        <Title>{displayReview ? t('Create a Review') : t('Create Image Post')}</Title>
      </Group>
      {currentUser?.muted ? (
        <Container size="xs">
          <Center p="xl">
            <Stack align="center">
              <AlertWithIcon color="yellow" icon={<IconLock />} iconSize={32} iconColor="yellow">
                <Text size="md">{t('You cannot create a post because your account has been muted.')}</Text>
              </AlertWithIcon>
            </Stack>
          </Center>
        </Container>
        ...
        <Stack spacing={8}>
          ...
          {versions && !reviewing && (
            <Select
              description={t('Select a resource to ensure that all uploaded images receive correct resource attribution')}
              placeholder={t('Select a resource')as string}
              value={selected}
          ...
          {displayReview && version && (
            <>
              <Input.Wrapper label={t('What did you think of this resource?')}>
                <Box mt={5}>
                  <EditUserResourceReview
              ...
              {currentUserReview && (
                <Text size="sm" color="dimmed">
                  {t(`We've saved your review. Now, consider adding images below to create a post showcasing the resource.`)}
                </Text>
              )}
            </>
          )}
