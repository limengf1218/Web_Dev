    ...
    <Container>
      {reviewing && !tags.length && (
        <DismissibleAlert
          ...
          color="blue"
          emoji="⭐️"
          content={t('To complete your review give your post a tag and hit publish.')}
        />
      )}
      <Grid gutter={30}>
        <Grid.Col md={4} sm={6} orderSm={2}>
          <Stack>
            <Group position="apart">
              <Title size="sm">{t('POST')}</Title>
              <Badge color={mutating > 0 ? 'yellow' : 'green'} size="lg">
                {mutating > 0 ? t('Saving') : t('Saved')}
              </Badge>
            </Group>
            <EditPostControls />
            <EditPostReviews />
