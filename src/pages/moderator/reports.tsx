    ...
    () => [
      {
        ...
        Cell: ({ row: { original: report } }) => (
          <Group spacing="xs" noWrap>
            ...
            <Tooltip label={t('Open reported item')} withArrow>
              <ActionIcon
                component="a"
...
const { t } = useTranslation();
  ...
  const updateReportMutation = trpc.report.update.useMutation({
    async onSuccess(results) {
      ...
      showSuccessNotification({
        ...
        message: t('Internal notes have been saved'),
      });
      if (mobile) onClose?.();
    },
    onError(error) {
  ...
  return (
    <Drawer
      ...
      title={`${upperFirst(type)} Report {t('Details')}`}
      size={mobile ? '100%' : 'xl'}
      padding="md"
      shadow="sm"
      zIndex={500}
    ...
    >
      {report && (
        <Stack>
          ...
          <Input.Wrapper
            label={t('Status')}
            description={t('Use this input to set the status of the report')}
          ...
          <Form form={form} onSubmit={handleSaveReport}>
            <Stack>
              <InputTextArea
                ...
                description={t('Leave an internal note for future reference (optional)')}
...
const { t } = useTranslation();
  ...
  if (typeof details === 'string' || typeof details === 'number' || typeof details === 'boolean')
    return <>{details}</>;
  if (Array.isArray(details)) return <>{t('Bad data')}</>;
...
const { t } = useTranslation();
  ...
  const { mutate, isLoading } = trpc.report.setStatus.useMutation({
    ...
    onError(error) {
      showErrorNotification({
        title: t('Failed to set report status')as string,
  ...
  return (
    <Menu>
      ...
      <Menu.Dropdown>
        <Menu.Label>Set {t('Status')}</Menu.Label>
        <Menu.Divider />
