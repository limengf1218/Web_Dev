...
const { t } = useTranslation();
  ...
  const [state, setState] = useState<State>({
    declineReason: t('Insufficient changes'),
    page: 1,
    opened: false,
    selectedModel: null,
  });
      ...
      { id: state.selectedModel.id, reason: state.declineReason },
      {
        ...
        onError(error) {
          showErrorNotification({
            title: t('Error declining request')as string,
  ...
  return (
    <Container size="sm">
      <Stack spacing={0} mb="xl">
        <Title order={1}>{t('Models Needing Review')}</Title>
        <Text size="sm" color="dimmed">
          {t('Unpublished models for violating ToS which their owners have request a review')}
        </Text>
      </Stack>
      {isLoading ? (
        <Center p="xl">
        ...
        <Stack>
          ...
          {pagination.totalPages > 1 && (
            <Group position="apart">
              <Text>{t('Total')} {pagination.totalItems} items</Text>
              <Pagination
                page={state.page}
          ...
          <Modal opened={state.opened} onClose={() => toggleModal()} title={t('Decline Request')}>
            <Stack>
              <Textarea
                name="declineReason"
                description={t('Reason for declining request')}
                minRows={2}
                placeholder="i.e.: {t('Insufficient changes')}"
        ...
        <Paper p="xl" withBorder>
          <Center>
            <Text size="md" color="dimmed">
              {t('There are no models that need review')}
            </Text>
          </Center>
        </Paper>
      )}
