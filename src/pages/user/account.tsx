...
import { BasicCard } from '~/components/Account/BasicCard';
...
export default function Account({ providers }: Props) {
  ...
  return (
    <>
      <Meta title={t('Manage your Account - mikomiko')} />

      <Container pb="md" size="xs">
        <Stack>
          <Stack spacing={0}>
            <Title order={1}>{t('Manage Account')}</Title>
            <Text color="dimmed" size="sm">
              {t('Take a moment to review your account information and preferences to personalize your experience on the site')}
            </Text>
          </Stack>
          <BasicCard />
          <ProfileCard />
