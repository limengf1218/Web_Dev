        ...
        { categoryId, modelIds: selection },
        {
          async onSuccess() {
            ...
            showSuccessNotification({
              title: 'Success',
              message: t('Successfully updated models category'),
            });
            await queryUtils.model.getWithCategoriesSimple.invalidate();
          },
          onError(error) {
            showErrorNotification({
              title: t('Failed to update models category')as string,
    ...
    <Container size="sm">
      <Stack>
              ...
              <tr>
                ...
                <th colSpan={3}>
                  <Group position="apart">
                    <Group spacing={4}>
                      <Text size="lg">{t('Model Category Manager')}</Text>
                    </Group>
                    <Group spacing={4}>
                      {selection.length > 0 && (
                        <Button
                      ...
                      <Menu width={200} withinPortal>
                        <Menu.Target>
                          <Button
                            ...
                            loading={isLoading}
                          >
                            {t('Set Category')} {selection.length > 0 && `(${selection.length})`}
                          </Button>
                        </Menu.Target>
                        <Menu.Dropdown>
                          {loadingCategories ? (
                            ...
                            <Center p="xs">
                              <Text color="dimmed">{t('You must select at least one model')}</Text>
                            </Center>
                          ) : (
                            categories
                          )}
        ...
        {pagination.totalPages > 1 && (
          <Group position="apart">
            <Text>{t('Total')} {pagination.totalItems.toLocaleString()} {t('items')}</Text>
